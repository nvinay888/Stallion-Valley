const HORSES = [
  ['Sulthan', 'Male · 4 years', 'warm', "Children’s favourite horse-mate — Sulthan is extremely playful."],
  ['Riya', 'Female · 6 years', 'oat', 'A well-trained, alert horse with superior reflexes — a preferred horse for experienced riders.'],
  ['Emma', 'Female · 3.5 years', 'warm', "Very active and loves long rides. Doesn’t like to trot and prefers experienced riders. Owned by Dr. Hormise Stephen, Rajagiri Hospital, Ernakulam."],
  ['Black Forest', 'Female · 4 years', 'dark', 'Tough and strong, very active and loves long rides. Owned by Mr. Joe Alappat, Francis Alappat Group.'],
  ['Lakshmi', 'Female · 10 years', 'oat', 'Adores children and is extremely loving — she has taken even 3-year-olds for rides.'],
  ['Bella', 'Female · 4 years', 'warm', 'Gorgeous-looking and extremely healthy, very friendly and gets along with most riders. Owned by Mr. Bobby Jose, Bodi Fit, Muvattupuzha.'],
  ['Romeo', 'Male · 4 years', 'dark', 'A caring horse, suited even for children.'],
  ['Sundance', 'Female · 2.5 years', 'oat', "Children’s favourite horse-mate — very playful. Owned by Mr. Vishal Menon, Ernakulam."],
];
    ...
    <div>
      <Section>
        ...
        <div style={{ display: 'grid', gridTemplateColumns: 'repeat(4, 1fr)', gap: 'var(--space-6) var(--space-5)' }}>
          {HORSES.map(([name, age, tn, note], i) => (
            <Reveal key={name} delay={(i % 4) * 110}>
              <ArchMedia label={name + ' portrait'} ratio="3 / 4" tone={tn} index={i + 1} />
      ...
      <FactsMarquee tone="sunken" items={[
        '10 horses boarded today, capacity for 10 more',
        '3 horses owned by the Academy itself',
        'Open 365 days for boarding visits',
        'Bordered by the Peechi–Vazhani Wildlife Sanctuary',
      ]} />

      <Section>
        <div style={{ display: 'grid', gridTemplateColumns: '5fr 7fr', gap: 'var(--space-8)', alignItems: 'start' }}>
          <div>
            ...
            {[
              ...
              <Reveal key={name} delay={i * 100}>
                <div style={{ background: 'var(--surface-raised)', border: '1px solid var(--border-hairline)', borderRadius: 'var(--radius-md)', padding: 'var(--space-5)', boxShadow: 'var(--shadow-sm)' }}>
                  ...
                  <div style={{ display: 'grid', gridTemplateColumns: '1fr 1fr', gap: '10px var(--space-5)' }}>
                    {items.map((it) => (
                  ...
                  <div style={{ marginTop: 'var(--space-5)', paddingTop: 'var(--space-4)', borderTop: '1px solid var(--border-hairline)', display: 'flex', justifyContent: 'space-between', alignItems: 'center' }}>
